...
Original file is located at
    https://colab.research.google.com/drive/1ZqfW9fu5GZkC0lZeglHWvm_HXlaP1t_z
"""
...
class MongolianTentSolver:
    ...
    def backtrack(self, order: List[int], index: int, max_label: int, start_time: float, time_limit: float) -> bool:
        ...
        # Checks if all of the vertices are labeled
        if index == len(order):
            # We verify solution validity by checking if unique used weights equals total edges
            if len(self.used_edge_weights) == self.num_edges:
                self.solution_found = True
                return True
            return False

        # Get the vertex to label
        vertex = order[index]

        # Find all possible label values for this vertex
        valid_labels = self.get_valid_labels(vertex, max_label)
        # Sort for greedy approach (try smaller values first)
        valid_labels.sort()
        ...
        for label in valid_labels:
            ...
            # Remove the weights that didnt work
            self.used_edge_weights.difference_update(new_weights)
            del self.labels[vertex]

        # If all valid labels fail, return False
        return False
    ...
    def visualize(self, figsize=(16, 10), show_edge_labels=True):
        ...
        all_unique = len(set(edge_weights)) == len(edge_weights)
    ...
    def _create_grid_layout(self) -> Dict[int, tuple]:
        """Create a grid layout with apex centered at top"""
        pos = {}
